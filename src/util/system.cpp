...
const char * const BITCOIN_CONF_FILENAME = "errorcoin.conf";
...
#else
    const char* pszModule = "errorcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "errorcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/errorcoin";
#else
    // Unix
    return pathRet / ".errorcoin";
#endif
#endif
}
