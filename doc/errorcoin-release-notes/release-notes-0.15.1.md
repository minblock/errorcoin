errorcoin Core version *0.15.1* is now available from:

  <https://download.errorcoin.org/errorcoin-0.15.1/>
...
A number of changes to the way errorcoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 lites per kB (0.00001 ETC). This is to prepare for dropping the minimum transaction fee to 0.1 lites per kB (0.0001 ETC) in the next release.
